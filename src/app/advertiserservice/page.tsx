  ...
  const fetchAdGroups = async () => {
    ...
    try {
      ...
      if (!response.ok || !data || (Array.isArray(data) && data.length === 0)) {
        ...
        const enabledAdGroups = enhancedAdGroups.filter(
          (group: Partial<AdGroup>) => group.status === "ENABLE"
        );
    ...
    try {
      ...
      if (result.data && Array.isArray(result.data)) {
        ...
        result.data.forEach((metric: Partial<AdGroup>) => {
          const adGroupIndex: number = newAdGroups.findIndex(
            (ag) => ag.adgroup_id === metric.adgroup_id
          );
          if (adGroupIndex !== -1) {
    ...
    ? adGroups
        ...
        .filter(
          (group) =>
            ...
            (group.adgroup_id &&
              String(group.adgroup_id).toLowerCase().includes(searchTerm.toLowerCase()))
        )
    : [];
        ...
        <div className="p-6">
          ...
          <div className="space-y-2 text-sm text-gray-600">
            ...
            <div className="flex items-center">
              ...
              <span
                className={`${parseFloat(formatNumber(group.cost_per_conversion)) > 10 ? "text-red-600 font-bold" : "text-green-600"}`}
              >
            ...
            <div className="flex items-center">
              ...
              <span
                className={`${parseFloat(formatNumber(group.cpc)) > 2 ? "text-red-600" : "text-green-600"}`}
              >
