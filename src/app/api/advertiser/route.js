...
const SUPABASE_API_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImptYmlibXVsd3pucmd0cmt3cnhrIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc0MDY4MzI4NywiZXhwIjoyMDU2MjU5Mjg3fQ.1j61oj07IGEa_pTPilCmHAZ3XrQkIRSZjrNh0GrkgjQ";
...
export async function GET(request) {
  ...
  try {
    ...
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/omgbeautybox_new?advertiser_id=eq.${advertiser_id}&status=eq.ENABLE`,
      {
        headers: {
...
async function getMetricsFromSupabase(advertiser_id) {
  ...
  try {
    ...
    const metricsResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/get_metrics?advertiser_id=eq.${advertiser_id}&order=created_at.desc`,
      {
        headers: {
          apikey: SUPABASE_API_KEY,
          Authorization: `Bearer ${SUPABASE_API_KEY}`,
