  ...
  try {
    ...
    if (role === "CREATOR") {
      if (!creatorHandleName) {
        return NextResponse.json({ error: "Creator handle name is required" }, { status: 400 });
      }
      ...
      if (existingHandleName) {
        return NextResponse.json({ error: "Creator handle name already taken" }, { status: 400 });
      }
    }
    ...
    const result = await prisma.$transaction(async (prisma) => {
      ...
      const user = await prisma.user.create({
        data: {
          ...
          role,
          creatorHandleName:
            role === "CREATOR" ? creatorHandleName : `brand-${randomUUID().slice(0, 8)}`,
