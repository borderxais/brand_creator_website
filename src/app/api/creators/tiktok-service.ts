...
export async function fetchTikTokCreatorData(
  access_token: string,
...
export async function saveCreatorData(creatorData: TikTokCreatorResponse["data"]) {
  try {
    ...
    const contentLabelName =
      creatorData.content_labels?.length > 0
        ? safeText(creatorData.content_labels[0].label_name)
        : null;

    const industryLabelName =
      creatorData.industry_labels?.length > 0
    ...
    const minimalData = {
      ...
      follower_count:
        typeof creatorData.followers_count === "number"
          ? Math.round(creatorData.followers_count)
          : null,
      creator_id: safeText(creatorData.creator_id),
      creator_price:
        typeof creatorData.creator_price === "number"
          ? Math.round(creatorData.creator_price)
          : null,
      currency: safeText(creatorData.currency),
      following_count:
        typeof creatorData.following_count === "number"
          ? Math.round(creatorData.following_count)
          : null,
      like_count:
        typeof creatorData.likes_count === "number" ? Math.round(creatorData.likes_count) : null,
      median_views:
        typeof creatorData.median_views === "number" ? Math.round(creatorData.median_views) : null,
      profile_image: safeText(creatorData.profile_image),
      videos_count:
        typeof creatorData.videos_count === "number" ? Math.round(creatorData.videos_count) : null,
      engagement_rate:
        typeof creatorData.engagement_rate === "number" ? creatorData.engagement_rate : null,
    };
