...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const response = await fetch(
      `${PYTHON_API_URL}/campaigns/brand/${brandId}/campaign/${campaignId}`,
      {
        method: "GET",
        headers: {
      ...
      if (contentType && contentType.includes("application/json")) {
        try {
          ...
          return NextResponse.json(
            { error: error.detail || "Failed to fetch campaign" },
            { status: response.status }
          );
        } catch (parseError) {
          console.error("Error parsing JSON response:", parseError);
          return NextResponse.json(
            { error: "Failed to fetch campaign - invalid response format" },
            { status: response.status }
          );
        }
      } else {
    ...
    if (!response.ok) {
      ...
      if (contentType && contentType.includes("application/json")) {
        try {
          const error = await response.json();
          return NextResponse.json(
            { error: error.detail || "Failed to update campaign" },
            { status: response.status }
          );
        } catch (parseError) {
          console.error("Error parsing JSON error response:", parseError);
          return NextResponse.json(
            { error: "Failed to update campaign - invalid response format" },
            { status: response.status }
          );
        }
      } else {
        ...
        console.error("Non-JSON error response:", errorText.substring(0, 200));
        return NextResponse.json(
          { error: "Failed to update campaign - server error" },
          { status: response.status }
        );
      }
    }
    ...
    if (!response.ok) {
      ...
      if (contentType && contentType.includes("application/json")) {
        try {
          ...
          console.error("Error parsing JSON error response:", parseError);
          return NextResponse.json(
            { error: "Failed to delete campaign - invalid response format" },
            { status: response.status }
          );
        }
      } else {
        ...
        console.error("Non-JSON error response:", errorText.substring(0, 200));
        return NextResponse.json(
          { error: "Failed to delete campaign - server error" },
          { status: response.status }
        );
      }
    }
