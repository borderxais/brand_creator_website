...
async def add_campaign(
    ...
    logger.info(
        f"Received campaign creation request for brand_id: {brand_id}, campaign title: {campaign.title}"
    )
...
async def ensure_brand_profile(brand_id: str = Path(..., description="The user ID of the brand")):
