...
async def get_campaigns(
    ...
    try:
        ...
        if not table_exists:
            logger.warning(
                "Campaigns table does not exist or is inaccessible, returning empty list"
            )
            return []
        ...
        try:
            ...
            if "permission denied" in error_msg.lower():
                logger.error(
                    "Permission denied error. Check your Supabase RLS policies and service key permissions."
                )
                return []
        ...
        for campaign in campaigns:
            if campaign.get("brand_id"):
                try:
                    ...
                    brand_response = (
                        supabase.table("BrandProfile")
                        .select("companyName")
                        .eq("id", campaign["brand_id"])
                        .execute()
                    )
                    if brand_response.data and len(brand_response.data) > 0:
                        campaign["brand_name"] = brand_response.data[0].get(
...
async def check_campaign_claim(
    ...
    try:
        ...
        creator_profile = (
            supabase.table("CreatorProfile").select("id").eq("userId", creatorId).execute()
        )
        ...
        logger.info(
            f"Checking if claim exists for creator {creator_id} and campaign {campaignId}: {claim_exists}"
        )
...
async def create_campaign_claim(request: Request, campaign_claim: CampaignClaimCreate):
    """Create a new campaign claim (application) from a creator."""
    try:
        logger.info(
            f"Creating campaign claim for user {campaign_claim.user_id} and campaign {campaign_claim.campaign_id}"
        )
        ...
        if not creator_profile.data or len(creator_profile.data) == 0:
            logger.error(
                f"Creator with userId {campaign_claim.user_id} not found in CreatorProfile table"
            )
        ...
        if existing_claim.data and len(existing_claim.data) > 0:
            logger.info(
                f"Creator {creator_id} has already applied to campaign {campaign_claim.campaign_id}"
            )
...
async def get_creator_campaign_claims(
    ...
    try:
        ...
        creator_profile = (
            supabase.table("CreatorProfile").select("id").eq("userId", creator_id).execute()
        )
        ...
        try:
            ...
            for claim in claims_response.data:
                ...
                try:
                    ...
                    logger.error(
                        f"Error fetching campaign for claim {claim.get('id')}: {str(db_error)}"
                    )
                ...
                result_item = {
                    ...
                    "primary_promotion_objectives": campaign_data.get(
                    ...
                    "creator_profile_preferences_gender": campaign_data.get(
                        "creator_profile_preferences_gender"
                    ),
                    "creator_profile_preference_ethnicity": campaign_data.get(
                        "creator_profile_preference_ethnicity"
                    ),
                    "creator_profile_preference_content_niche": campaign_data.get(
                        "creator_profile_preference_content_niche"
                    ),
                    "preferred_creator_location": campaign_data.get("preferred_creator_location"),
                    "language_requirement_for_creators": campaign_data.get(
                ...
                if campaign_data.get("brand_id"):
                    try:
                        brand_response = (
                            supabase.table("BrandProfile")
                            .select("companyName")
                            .eq("id", campaign_data["brand_id"])
                            .execute()
                        )
                        if brand_response.data and len(brand_response.data) > 0:
                            result_item["campaign_brand_name"] = brand_response.data[0].get(
                                "companyName", "Unknown Brand"
                            )
                        else:
                            result_item["campaign_brand_name"] = (
                                f"Brand {campaign_data['brand_id']}"
                            )
    ...
    try:
        ...
        for campaign in campaigns:
            try:
                ...
                logger.info(
                    f"Found {len(campaign['applications'])} claims/applications for campaign ID: {campaign['id']}"
                )
                ...
                for claim in campaign["applications"]:
                    try:
                        ...
                        logger.error(
                            f"Error fetching creator details for claim {claim.get('id')}: {str(e)}"
                        )
        ...
        try:
            brand_response = supabase.table("BrandProfile").select("*").eq("id", brand_id).execute()
...
async def get_brand_campaign(
    ...
    try:
        ...
        brand_profile_response = (
            supabase.table("BrandProfile").select("id").eq("id", brand_id).execute()
        )
        ...
        if brand_profile_response.data and len(brand_profile_response.data) > 0:
            ...
            user_brand_response = (
                supabase.table("BrandProfile").select("id").eq("userId", brand_id).execute()
            )
        ...
        try:
                                ...
                                try:
                                    ...
                                    if user_response.data and len(user_response.data) > 0:
                                        ...
                                        application["creator"]["user"] = (
                                            user_data  # Include full user data
                                        )
                                except Exception as user_error:
                                    logger.error(
                                        f"Error fetching user data for creator {creator_data['userId']}: {str(user_error)}"
                                    )

                except Exception as e:
                    logger.error(
                        f"Error fetching creator details for application {application.get('id')}: {str(e)}"
                    )
            ...
            if "invalid input syntax for type uuid" in str(e).lower():
                logger.warning(
                    f"UUID format issue fetching applications for campaign {campaign_id}"
                )
            campaign["applications"] = []
...
async def add_campaign(
    ...
    try:
        ...
        logger.info(
            f"New fields - script_required: {campaign_data.get('script_required')}, product_name: {campaign_data.get('product_name')}, paid_promotion_type: {campaign_data.get('paid_promotion_type')}"
        )
...
async def update_campaign(
    ...
    try:
        ...
        campaign_data = {
            k: v for k, v in campaign_data.items() if v is not None and v != "" and k != "brand_id"
        }
...
async def get_campaign_by_id(campaign_id: str = Path(..., description="The ID of the campaign")):
    """
